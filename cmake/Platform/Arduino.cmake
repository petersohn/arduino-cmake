...
function(GENERATE_AVR_FIRMWARE INPUT_NAME)
  ...
  parse_generator_arguments(${INPUT_NAME} INPUT
      "NO_AUTOLIBS;MANUAL"            # Options
      "BOARD;CPU;PORT;PROGRAMMER"  # One Value Keywords
...
function(GENERATE_ARDUINO_EXAMPLE INPUT_NAME)
  parse_generator_arguments(${INPUT_NAME} INPUT
      ""                                       # Options
      "LIBRARY;EXAMPLE;BOARD;CPU;PORT;PROGRAMMER"  # One Value Keywords
...
function(REGISTER_HARDWARE_PLATFORM PLATFORM_PATH)
  message(STATUS "Registering hardware platform ${PLATFORM_PATH}")
      ...
      if (${PLATFORM}_VARIANTS_PATH)
        ...
        foreach (dir ${sub-dir})
          if (IS_DIRECTORY ${dir})
            ...
            set(${variant}_variant.path ${dir} CACHE INTERNAL "The path to the variant ${variant}")
          endif ()
        endforeach ()
      endif ()
      ...
      if (${PLATFORM}_CORES_PATH)
        ...
        foreach (dir ${sub-dir})
          if (IS_DIRECTORY ${dir})
            ...
            set(${core}_core.path ${dir} CACHE INTERNAL "The path to the core ${core}")
          endif ()
        endforeach ()
      endif ()
...
function(get_arduino_flags COMPILE_FLAGS_VAR LINK_FLAGS_VAR BOARD_ID MANUAL)
  ...
  if (BOARD_CORE)
    ...
    if (${${BOARD_ID}_CPUMENU}.build.f_cpu)
      set(COMPILE_FLAGS "${COMPILE_FLAGS} -DF_CPU=${${${BOARD_ID}_CPUMENU}.build.f_cpu}")
    else ()
    ...
    set(COMPILE_FLAGS "${COMPILE_FLAGS} -DARDUINO=${ARDUINO_VERSION_DEFINE} -DARDUINO_${${BOARD_ID}.build.board} -DARDUINO_ARCH_AVR -mmcu=${${${BOARD_ID}_CPUMENU}.build.mcu}")
...
function(setup_arduino_target TARGET_NAME BOARD_ID ALL_SRCS ALL_LIBS COMPILE_FLAGS LINK_FLAGS MANUAL)
  ...
  add_custom_command(TARGET ${TARGET_NAME} POST_BUILD
      ...
      -DMCU=${${${BOARD_ID}_CPUMENU}.build.mcu}
...
function(setup_arduino_bootloader_burn TARGET_NAME BOARD_ID PROGRAMMER PORT AVRDUDE_FLAGS)
  ...
  if (NOT EXISTS "${ARDUINO_BOOTLOADERS_PATH}/${${BOARD_ID}.bootloader.path}/${${${BOARD_ID}_CPUMENU}.bootloader.file}")
    message("${ARDUINO_BOOTLOADERS_PATH}/${${BOARD_ID}.bootloader.path}/${${${BOARD_ID}_CPUMENU}.bootloader.file}")
  ...
  if (${BOARD_ID}.bootloader.extended_fuses)
    list(APPEND AVRDUDE_ARGS "-Uefuse:w:${${${BOARD_ID}_CPUMENU}.bootloader.extended_fuses}:m")
  endif ()
  list(APPEND AVRDUDE_ARGS
      "-Uhfuse:w:${${${BOARD_ID}_CPUMENU}.bootloader.high_fuses}:m"
      "-Ulfuse:w:${${BOARD_ID}.bootloader.low_fuses}:m")

  # Set bootloader image
  list(APPEND AVRDUDE_ARGS "-Uflash:w:${${${BOARD_ID}_CPUMENU}.bootloader.file}:i")
...
function(setup_arduino_programmer_args BOARD_ID PROGRAMMER TARGET_NAME PORT AVRDUDE_FLAGS OUTPUT_VAR)
  ...
  list(APPEND AVRDUDE_ARGS "-p${${${BOARD_ID}_CPUMENU}.build.mcu}")  # MCU Type
...
function(setup_arduino_bootloader_args BOARD_ID TARGET_NAME PORT AVRDUDE_FLAGS OUTPUT_VAR)
  ...
  list(APPEND AVRDUDE_ARGS
      ...
      "-p${${${BOARD_ID}_CPUMENU}.build.mcu}"        # MCU Type
      )

  # Programmer
  if (NOT ${${BOARD_ID}_CPUMENU}.upload.protocol OR ${${BOARD_ID}_CPUMENU}.upload.protocol STREQUAL "stk500")
    ...
    list(APPEND AVRDUDE_ARGS "-c${${${BOARD_ID}_CPUMENU}.upload.protocol}")
  endif ()

  set(UPLOAD_SPEED "19200")
  if (${${BOARD_ID}_CPUMENU}.upload.speed)
    set(UPLOAD_SPEED ${${${BOARD_ID}_CPUMENU}.upload.speed})
  endif ()
...
if (NOT ARDUINO_FOUND AND ARDUINO_SDK_PATH)
  register_hardware_platform(${ARDUINO_HARDWARE_PATH}/arduino/)
...
if (ARDUINO_SDK_VERSION VERSION_LESS 1.5)
  ...
  if (NOT ARDUINO_PLATFORM)
    register_hardware_platform(${ARDUINO_HARDWARE_PATH}/${ARDUINO_HARDWARE_VENDOR}/avr)
    set(ARDUINO_PLATFORM "AVR")
  else ()
    string(TOLOWER ${ARDUINO_PLATFORM} _platform)
    register_hardware_platform(${ARDUINO_HARDWARE_PATH}/${ARDUINO_HARDWARE_VENDOR}/${_platform})
  endif ()
endif ()
