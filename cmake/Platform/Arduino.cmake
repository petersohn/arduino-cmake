...
function(get_arduino_flags COMPILE_FLAGS_VAR LINK_FLAGS_VAR BOARD_ID MANUAL)
    ...
    if(BOARD_CORE)
        if(ARDUINO_SDK_VERSION MATCHES "([0-9]+)[.]([0-9]+)([.]([0-9]+))?")
            ...
            if(CMAKE_MATCH_4 GREATER 10)
                set(ARDUINO_VERSION_DEFINE "${ARDUINO_VERSION_DEFINE}${CMAKE_MATCH_4}")
            else()
                set(ARDUINO_VERSION_DEFINE "${ARDUINO_VERSION_DEFINE}0${CMAKE_MATCH_4}")
            endif()
        else()
        ...
        # output
        set(COMPILE_FLAGS "-DF_CPU=${${BOARD_ID}.build.f_cpu} -DARDUINO=${ARDUINO_VERSION_DEFINE} -mmcu=${${BOARD_ID}${ARDUINO_CPUMENU}.build.mcu} '-DUSB_PRODUCT=\"${${BOARD_ID}.name}\"' -fno-threadsafe-statics")
    ...
    if(NOT TARGET ${TARGET_LIB_NAME})
        ...
        if(LIB_SRCS)
            ...
            foreach(LIB_DEP ${LIB_DEPS})
                message(STATUS "Dep of lib ${LIB_NAME}: ${LIB_DEP}")
            ...
            list(INSERT LIB_TARGETS 0 ${TARGET_LIB_NAME})
...
# [PRIVATE/INTERNAL]
#
# setup_arduino_target(TARGET_NAME BOARD_ID ALL_SRCS ALL_LIBS COMPILE_FLAGS LINK_FLAGS MANUAL)
#
